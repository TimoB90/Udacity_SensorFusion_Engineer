...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr &viewer,ProcessPointClouds<pcl::PointXYZI> *pointProcessorI, const pcl::PointCloud<pcl::PointXYZI>::Ptr &inputCloud) 
{
	...
	pcl::PointCloud<pcl::PointXYZI>::Ptr filterCloud = pointProcessorI->FilterCloud( inputCloud, 0.2, Eigen::Vector4f(-(30 / 2), -6, -2.5, 1), Eigen::Vector4f(30, 6.5, 2.5, 1));
    //renderPointCloud(viewer, filteredCloud, "filteredCloud");

    const std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud = pointProcessorI->RansacPlane(filterCloud, 20, 0.2);
    ...
    const std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessorI->Clustering(segmentCloud.first, 0.3, 10, 1500);


    if (cloudClusters.empty()) { return; }

    int clusterId{ 0 }, colorIndex{ 0 };
    const std::vector<Color> colors{ Color(1, 0, 0), Color(1, 1, 0), Color(0, 0, 1) };
