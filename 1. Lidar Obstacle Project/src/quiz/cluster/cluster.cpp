 ...
 void clusterHelper(int indice, const std::vector<std::vector<float>>& points, std::vector<int>& cluster,  std::vector<bool>& processed, KdTree* tree, float distanceTol)
  {
  		processed[indice] = true;
   		...
   		std::vector<int> nearestpoint = tree->search(points[indice], distanceTol);
...
int main ()
{
	...
	// Create data
	std::vector<std::vector<float>> points = { {-6.2,7,20}, {-6.3,8.4,2}, {-5.2,7.1,1}, {-5.7,6.3,1}, {7.2,6.1,1}, {8.0,5.3,1}, {7.2,7.1,1}, {0.2,-7.1,1}, {1.7,-6.9,1}, {-1.2,-7.2,1}, {2.2,-8.9,1} };
    ...
    //	tree->insert(points[i],i); 
     ...
     for (auto point : cloud->points) {
  	...
  	std::vector<int> nearby = tree->search({-6,7,0.5},3.0);
